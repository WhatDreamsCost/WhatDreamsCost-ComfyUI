...
class LTXDirectorGuide(LTXVAddGuide):
    ...
    def execute(cls, positive, negative, vae, latent, guide_data, scale_by=1.0, upscale_method="bicubic", kf_softness=0.0) -> io.NodeOutput:
        ...
        for idx, img_tensor in enumerate(images):
            ...
            # this slot to the keyframe content — exact image at exact time. Other frames attend
            # to this slot via the model's natural RoPE-decayed self-attention, so kf influence
            # falls off smoothly with temporal distance instead of being a hard pull. No appended
            # tokens, no doubled-up RoPE, no artificial dominance.
